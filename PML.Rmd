...
pml_train <- read.csv("https://d396qusza40orc.cloudfront.net/predmachlearn/pml-training.csv",header=TRUE)
newTrain <- pml_train[,colSums(is.na(pml_train))==0] # Deleting blank columns
newTrain <- newTrain[,-c(1:7)] # Deleting the first 7 columns since they are useless
pml_test <- read.csv("https://d396qusza40orc.cloudfront.net/predmachlearn/pml-testing.csv",header=TRUE)
newTest <- pml_test[,colSums(is.na(pml_train))==0] # Deleting blank columns
